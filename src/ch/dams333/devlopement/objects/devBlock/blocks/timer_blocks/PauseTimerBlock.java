...
public class PauseTimerBlock extends DevBlock{

    private UUID timerUUID;

    public PauseTimerBlock(Devlopement main, Location loc) {
        super(main, loc);
        super.setType(BlockType.PAUSE_TIMER);
        timerUUID = null;
    }

    public PauseTimerBlock(File file, Devlopement main) {
        super(file, main);
        super.setType(BlockType.PAUSE_TIMER);
    ...
    public BlockType getBlockType() {
        return BlockType.PAUSE_TIMER;
    }

    @Override
    public void serialize() {
        File blockFile = new File(super.getMain().getDataFolder(), "blocks/timer/pause_timer/" + super.getUuid().toString() + ".yml");
    ...
    public void execute(CodeExecutor codeExecutor) {
        ...
        if(getMain().gameTimerManager.isTimerStarted(timerUUID)){
            getMain().gameTimerManager.pauseTimer(timerUUID);
        }
